...
class Tree(Nodes.Chain):
    ...
    def get_subset_tree(self,taxa):
        import copy
        '''given a list 'taxa', checks that all taxa are present in self,

        prunes all taxa in self NOT present in 'taxa' '''
        unmatched = [t for t in taxa if t not in self.get_taxa()]
        if unmatched:
            raise TreeError, "%s not in tree" % (", ".join(unmatched), )
        else:
            new_tree = copy.deepcopy(self)
            to_prune = [t for t in self.get_taxa() if t not in taxa]
            for t in to_prune: new_tree.prune(t)
            return new_tree
    ...
    def to_string(self,support_as_branchlengths=False,branchlengths_only=False,plain=True,plain_newick=False):
        ...
        for newick with branchlengths, need to use plain_newick=True, plain=False
        """
    ...
    def calculate_evidence_weight(self, source, target):
        '''given a source and target leaf pair (either names or node ids)
        returns a tuple of calculated evidence weights (lca-source and lca-target) as follows

        sum of weights for each branch from the common ancestor to both leaves
        weight = branchlength / number of other species below parent in the tree

        NOTE: to be used only with trees already "subtree'd" to contain on evidence-bearing leaves
        (try newtree = tree.get_subset_tree(['dmel','dpse']) first)
        '''

        def anc_to_leaf_weight(tree,anc,leaf):
            internodes = [anc] + tree.trace(anc,leaf) # hack to include lca
            weights = []
            for i, node in list(enumerate(internodes))[:-1]:
                down = internodes[i+1]
                bl = tree.distance(node, down)
                sp_under = len(filter(None,[tree.is_parent_of(down,n) for n in tree.get_terminals()]))
                weights.append( bl / sp_under )
            return weights
            

        if isinstance(source, str):
            source = self.search_taxon(source)
        if isinstance(target, str):
            target = self.search_taxon(target)

        lca = self.common_ancestor(source,target)
        
        source_side = anc_to_leaf_weight(self,lca,source)
        target_side = anc_to_leaf_weight(self,lca,target)

        return (sum(source_side),sum(target_side))
            
        

def reorder_newick(newick_string, new_first):
    '''takes a newick string and re-orders as necessary to put new_first species first
    '''
    import re

    def find_splitcomma(newick_string):
        par = 0
        for i,c in enumerate(newick_string):
            if c == '(':
                par += 1
            if c == ')':
                par -= 1
    ...
    newick_string = newick_string.strip(';')
    #print newick_string
    if re.search(',',newick_string):
        newick_string = newick_string[1:-1] # unwrap outer parens
        #print 'parens unwrapped:',newick_string
        if newick_string.startswith('('):
            sc = find_splitcomma(newick_string)
        else:
            sc = newick_string.find(',')
        left = newick_string[:sc]
        right = newick_string[sc+1:]

        #print 'comma found at %s, split to %s and %s' % (sc,left,right)
        if re.search(new_first,left):
            ret = '(%s,%s)' % (reorder_newick(left,new_first),right)
        #    print ret
            return ret
        elif re.search(new_first,right):
            ret = '(%s,%s)' % (reorder_newick(right,new_first),left)
        #    print ret
            return ret
        else:
            raise ValueError, '%s not found in either %s or %s' % (new_first,left,right)
    else:
        #print newick_string
